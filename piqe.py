...
def compute_piqe_score(image):
    image_rgb = image.cpu().detach().numpy().transpose(1, 2, 0)  # Move to CPU and convert to numpy array, then transpose
    score, _, _, _ = piqe(image_rgb)
    return score

def calculate_average_piqe_score(image_tensor):
    num_images = image_tensor.shape[0]
    total_score = 0

    for i in range(num_images):
        score = compute_piqe_score(image_tensor[i])
        total_score += score
