        ...
        for iter_idx, (img_lr, img_gt, filename) in enumerate(tta_dataloader):
            ...
            if args.save_results:  
                ...
                for i in range(img_out.size(0)):
                    single_img = img_out[i]
                    single_img = single_img.byte().permute(1, 2, 0).cpu()
                    # os.makedirs(f'{args.save_dir}/{corruption}', exist_ok=True)                    
                    # save_path = os.path.join(args.save_dir, f"{corruption}/{filename[i]}.png")
                    # util.write_img(save_path, single_img.cpu().numpy())
                    util.write_img(os.path.join(args.save_dir, filename[i] + '.png'), single_img.numpy())
                    print(f"Image {i} saved at {args.save_dir}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metric_file', type=str, default='tta_metrics_aj.csv', help='path to the file for recording metric')
    ...
    parser.add_argument('--pre_train', type=str, default='/mnt/sdb1/aditi/SRTTA/checkpoints/EDSR_baseline_x2.pt', help='pre-trained model directory')
    parser.add_argument('--classifier', type=str, default='/mnt/sdb1/aditi/SRTTA/checkpoints/classifier.pt', help='pre-trained model directory')
    ...
    args.save_dir = "/mnt/sdb1/aditi/SRTTA/Result_images_fbcnn/original_q_Triplet_DIV2KC_2x_aj"
