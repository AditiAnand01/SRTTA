...
def run(args):
    ...
    model = EDSR(args).to(device) #modified
    print("1...",model)
    corruptions = []
    ...
    origin_model = EDSR(args).to(device) #modified
    print("2...",origin_model)
    origin_model = util.load_model(args.base_model, origin_model)
    print("3...",origin_model)
    exit()
    if args.cls_model:
        cls_model=Classifier().eval().to(device) #modified
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pre_train', type=str, default='C:\VSCode\SRTTA\checkpoints\EDSR_baseline_x4.pt', 
                        help='pre-trained model directory')
    parser.add_argument('--dir_data', type=str, default='C:\VSCode\SRTTA\datasets', help='dataset directory')
    parser.add_argument('--base_model', type=str, default='C:\VSCode\SRTTA\checkpoints\EDSR_baseline_x4.pt', help='path to base model')
    ...
    parser.add_argument('--n_threads', type=int, default=4, help='number of threads for data loading') #earlier default=12
