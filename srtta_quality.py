...
from piqe import calculate_average_piqe_score
...
class SRTTA():
    def __init__(self, args, model, optimizer, fisher=None, cls_model=None):
        ...
        # Initialize a DataFrame to store PSNR and SSIM values
        self.metrics_df = pd.DataFrame(columns=['img_name', 'iteration', 'corruption', 'psnr', 'ssim'])
    ...
    def extract_vgg16_features(self, image_batch):
        """
        Extract features from a batch of images using a modified VGG16 model.

        Parameters:
        image_batch (torch.Tensor): Input image batch as a PyTorch tensor with shape (batch_size, 3, H, W).

        Returns:
        torch.Tensor: Extracted features as a PyTorch tensor with shape (batch_size, feature_dim).
        """
        # Ensure the input data is a tensor with the correct shape (batch_size, 3, H, W)
        if not isinstance(image_batch, torch.Tensor) or image_batch.dim() != 4 or image_batch.size(1) != 3:
            raise ValueError("Input data should be a 4D PyTorch tensor with shape (batch_size, 3, H, W).")

        # Define the transform to resize images to 224x224
        transform = transforms.Compose([
            transforms.Resize((224, 224))
        ])

        # Apply the transform to each image in the batch
        image_batch_resized = torch.stack([transform(image) for image in image_batch])

        # Load pre-trained VGG16 model
        vgg16_model = models.vgg16(pretrained=True)

        # Modify the classifier to exclude the last layer
        vgg16_model.classifier = nn.Sequential(*list(vgg16_model.classifier.children())[:-1])

        # Set the model to evaluation mode
        vgg16_model.eval()

        # Move the model and the input to the appropriate device (CPU or GPU)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        vgg16_model.to(device)
        image_batch_resized = image_batch_resized.to(device)

        # Pass the batch of images through the modified VGG16 model
        with torch.no_grad():
            features_var = vgg16_model(image_batch_resized)

        # Extract features
        features = features_var.detach()

        return features

    def average_cosine_similarity(self,vectors1, vectors2):
        """
        Calculate the average cosine similarity between two batches of feature vectors.

        Parameters:
        vectors1 (torch.Tensor): First batch of feature vectors of shape (32, 4096).
        vectors2 (torch.Tensor): Second batch of feature vectors of shape (32, 4096).

        Returns:
        float: The average cosine similarity between the two batches of vectors.
        """
        assert vectors1.shape == vectors2.shape, "Input tensors must have the same shape"
        cos_sim = F.cosine_similarity(vectors1, vectors2, dim=1)
        avg_cos_sim = cos_sim.mean().item()
        return avg_cos_sim

    def cosine_loss(self,fv_c,fv_d):
        loss = 1 - self.average_cosine_similarity(fv_c,fv_d)
        return torch.tensor(loss, requires_grad=True)
    
    def quality_loss_piqe(self, img):
        iqa_score = calculate_average_piqe_score(img)
        loss = (iqa_score/100)
        return torch.tensor(loss, requires_grad=True)
    ...
    def test_time_adaptation(self, train_loader, corruption=None):
        ...
        if corruption is not None:
            ...
            for corruption_ in corruption:
                if corruption_.lower().find('noise') >= 0:
                    task_idx = 1
                elif corruption_.lower().find('blur') >= 0:
                    task_idx = 2
        ...
        out_stu_normalized = self.normalize(out_student_gt)
        out_stu_in_normalized = self.normalize(out_student)

        fv_d = self.extract_vgg16_features(out_student)
        fv_c = self.extract_vgg16_features(out_tea_gt)

        loss_cosine = self.cosine_loss(fv_c,fv_d)
        q_out_stu_gt = self.quality_loss_piqe(out_stu_normalized)
        q_out_stu = self.quality_loss_piqe(out_stu_in_normalized)
        loss_quality = q_out_stu_gt - q_out_stu
        
        loss1 = 0.75 * loss_cosine + loss_quality 


        # compute student loss
        loss2 = self.compute_loss(out_student, out_student_gt) 
        # compute teacher loss
        if self.args.teacher_weight > 0:
            loss2 += self.args.teacher_weight * self.compute_loss(out_student, out_tea_gt) 
            loss2 = self.args.teacher_weight * self.cosine_loss(fv_d, fv_c) 

        loss = loss_quality + loss2
    ...
    def save(self, corruption='GaussianBlur', iter_idx=0):
        ...
        # Save the DataFrame to a CSV file
        csv_filename = "/mnt/sdb1/aditi/SRTTA/Metrics_fbcnn/original_q_Triplet_DIV2KC_2x_aj.csv"
        self.save_metrics_to_csv(csv_filename)